 ...
 */
  struct proc *next_p, *temp_p;
  int cur_group;
  int i;
  if (rdy_head[USER_Q] == NIL_PROC) return;
  
  cur_group = rdy_head[USER_Q]->group;
  if(++counter <= groupsQuants[cur_group] ){ /*process still have time*/
     pick_proc();
     return;
  }
  
  /* we have only 1 proc in queue*/
  ...
  /*quant is finished*/
  /* move head at the end of queue*/
  rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
  rdy_tail[USER_Q] = rdy_head[USER_Q];
  rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
  rdy_tail[USER_Q]->p_nextready = NIL_PROC;

  /*searching for next group proc*/ 
  for( i = 0; i<NRGROUPS; i++){
 
  /*switch to the next group*/
  switch( cur_group ){
  case 0 : ++cur_group;   break;
  case 1 : ++cur_group;   break;
  case 2 : cur_group = 0; break;
  }
  
  /* start from head*/
  next_p = rdy_head[USER_Q] ;
  
  if(next_p->group == cur_group) break; /*found it on head */
  
  while(next_p->p_nextready->group != cur_group 
        && next_p->p_nextready != NIL_PROC)
        next_p = next_p->p_nextready;
  
  
  if(next_p ->p_nextready == NIL_PROC)/* next group is absent */
     continue;
  
  /* next_p -> p_nextready is process that we need
     put it on the head of queue */ 
    
  temp_p = next_p->p_nextready;
  next_p->p_nextready = temp_p->p_nextready;
  temp_p->p_nextready = rdy_head[USER_Q];
  rdy_head[USER_Q] = temp_p;
  break;
  }/*for*/
